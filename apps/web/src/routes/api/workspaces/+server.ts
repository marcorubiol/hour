...
type WorkspaceItem = Pick<
  ...
  | 'logo_url'
> & {
  // ADR-066 — optional pretty URL alias. Typed by hand until db-types
...
const CreateBodySchema = v.object({
  ...
  // ADR-066: no client-chosen slug — identity is a machine short-id
...
export const GET: RequestHandler = async ({ request, url, platform, locals }) => {
  ...
  // TODO(ADR-066): add `alias` to this select once the

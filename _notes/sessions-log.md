...
**Lo que quedó CANÓNICO** (ya en `_decisions.md`, no hay que volver a pensarlo):
- **ADR-085** — el `invitat`: una membresía con permisos, **menos el login, más una fecha de fin**. No es mecanismo nuevo. No debilita ADR-082 §6, lo hace literal: lo que cae es el supuesto tácito de que *estar dentro exigía login*. El muro nunca fue la membresía, es el login y la plaza. Escribe, es parte del hilo, ve el historial entero, se le invita a **facetas de un contenedor** (no a hilos sueltos), un hilo libre nunca hereda invitados, revocar no borra, y muere con el contenedor. Más **delegación acotada por el delegante** en los tres ejes (faceta · verbo · nivel), con lo que tienes como **techo, nunca como defecto** — eso sustituye a la capacidad "invitar" como gate suelto.
- **Enmienda a ADR-082** — las facetas existen **por nivel de contenedor**. Las dos listas que 082 y 083 esbozaban nunca fueron dos listas: eran **una lista vista a dos alturas** (082 gatea lo de arriba, 083 habla de lo de abajo; Diners salía en las dos porque es la única que cruza los cuatro niveles). `General` **no es faceta**: es la pertenencia al contenedor. **Absent ≠ denied**. Presets renombrados: `Convidat`→**`Mínim`**, `Producció`→**`Coordinació`**.
- **Enmienda a ADR-083** — audiencia de cualquier hilo = `derived(faceta, contenedor) ∪ invitats`. **Una sola fórmula** para hilos de faceta y libres (el `derived` de un libre es vacío). Sustituye a los dos modelos de audiencia de §5.
- **`build/structure-model.md`** — **dos escaleras, no una**. La palabra "nivel" hacía dos trabajos: *edición* (espai · projecte · línia→mòduls — tres, intacta) y *alcance/acceso* (espai · projecte · línia · **performance** — cuatro). El bolo siempre estuvo en la cadena; lo que faltaba era decir que puedes entrar en él. **No toca la regla de módulos**: el performance lleva acceso y conversación, no compone módulos. Y **la faceta entra como primitivo** — el único que corre *a través* de la escalera en vez de sentarse en ella.

**Lo que se pensó y se descartó:**
- **Producció como faceta** — dibujada, probada y **caída**. Tres cosas: al escribir un hilo de Producció realista todos los mensajes se iban a técnica/logística/diners; su contenido ya tiene destino y el residuo se llama `General`; y **decisivo**, en el código desplegado `ProductionStub.svelte` **ES el contenedor** de logistics/hospitality/technical — ponerla como hermana suya era error de categoría. Sobrevive como **rol**, que es lo que siempre fue. (Marco la quiso mantener primero; la evidencia del código la tumbó.)
- **La rebanada fina** (hilos sin facetas primero, facetas después con evidencia de una temporada) — propuesta por .zerø, **rechazada por Marco**: quiere hilos con facetas ya. Asumido a sabiendas: la tabla de facetas se construye antes de la temporada que debía validarla, y lo que lo hace tolerable es que **la lista es datos, no código**.
- **Comms como quinto tipo en el Desk** — descartado a favor de la idea de Marco: **sube a la lente Conversations**, con el patrón de ADR-076 (una puerta, dos proyecciones: *amb qui parles* / *de què es parla*). Razón estructural: cada concern del modelo tiene lente + rebanada urgente en el Desk; comms era **el único concern sin lente**, y por eso se comportaba como isla. ADR-083 decidió dónde VIVEN los hilos y nunca dónde se LEEN en agregado — ese era el hueco. **Pendiente de ADR.**
- **Membresía aditiva** (4 tablas, una por nivel) — descartada a favor de **una membresía polimórfica** (opción B), por etapas.

**El diseño** (`app design/`, con `index.html` como puerta): 7 prototipos que enlazan el `base.css` REAL, en claro y oscuro. La página del bolo (`performance-page-prototype.html`) es la vigente y la más decidida; `revoke-access-prototype.html` cubre el hueco que nadie había dibujado. Convergió **el bastidor de tres carriles** (marginalia · partitura · carril) y la corrección clave: **lo que lleva el carril es por pantalla** — *centro = lo que haces y cambia · carril = lo que consultas y simplemente es cierto*. El roster de permisos pasa a ser **puerta**, y lo que no se puede plegar nunca es la **línea de audiencia por hilo**, que es lo que sostiene la transparencia de ADR-083 §5.

**Gotchas que costaron tiempo (todas errores propios, para no repetirlos):**
- **El vacío de 144px.** `base.css` da a todo `<section>` de primer nivel `padding-block: var(--section-padding-block)` (~72px) + `padding-inline`. Es ritmo editorial, correcto para login y páginas públicas; dentro de la app inyecta ~144px de aire muerto entre secciones. La app ya lo desactiva (`.shell__content :where(section:not(section section))`), con un comentario que describe el mismo síntoma. **Un prototipo ES contexto de shell y tiene que reproducir el opt-out o miente sobre el ritmo real.** Era buena parte del "mucho espacio vertical" que Marco veía.
- **Medir contraste es delicado y me equivoqué dos veces.** `color-mix(in oklch, …)` computa a **`oklab()`**, no a `rgb()` — una regex ingenua da 1:1 para texto normal. Y `getPropertyValue('--token')` devuelve el **token sin resolver** (`var(--otro)`), que el canvas se come sin avisar. Resultado: reporté cifras contradictorias y **no se tocó `tokens.css`**. Lo único sólido, y es de inspección: `--text-muted` es el único token de tinta definido por **alfa** y el bloque oscuro no lo re-ancla, a diferencia de toda la escala. **Medir con Lighthouse/axe contra la app corriendo antes de cambiar nada.**
- **El puente user↔person YA EXISTÍA y no lo vi.** `user_profile.person_id` con índice único (ADR-072 §6, `2026-07-18_user_profile_person_id.sql`, **sin aplicar** — por eso no salía en `schema.sql`). La primera versión de la migración lo duplicaba en `membership`, creando dos respuestas a "qué persona es este usuario". Corregido: identidad **siempre** por `user_profile.person_id`; `membership.person_id` dice para quién es la membresía, nunca quién llama. **Cuatro de los siete bloqueantes del review salían de ahí.**
- **`performance.line_id` es NULLABLE.** La primera `container_chain` subía performance→line→project y un bolo sin línea perdía proyecto y espacio — y con ellos todos los permisos concedidos por encima. Se reescribió para leer las FK denormalizadas que `performance`, `line` y `conversation` ya llevan.
- Renombres: esta base ha hecho `show`→`performance`, `line`→`section`→`line`, `engagement`→`conversation`. **No fiarse de ningún nombre sin verificarlo contra el schema de hoy.**

**Verificación**: los 7 prototipos renderizados en claro y oscuro con el chromium instalado, auditados por recorte y desbordamiento (0 en ambos). Migración **revisada adversarialmente** por 4 lentes (seguridad · SQL · fidelidad al modelo · seguridad de migración): 32 hallazgos, 7 bloqueantes → `_notes/review-comms-migration-2026-07-20.md`. Cinco bloqueantes cerrados en la propia migración.

**Queda** — ver `_tasks.md § Comms`. Lo importante: **nada aplicado a `hour-phase0`**, y **dos decisiones de arquitectura** antes de poder aplicar (cómo se autentica un invitado, y el RPC de expansión de presets sin el cual la etapa 1 nace muerta).

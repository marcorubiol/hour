...
- [ ] **Desk v2 — EN CURSO** (sesión paralela, aún fuera de este checkout). En el árbol hoy = v1: 2 de 4 fuentes (`DeskBoard` conversations + sección `TaskBoard`) en secciones SEPARADAS, no feed mixto rankeado. Falta: performances como headline de bucket, money alerts (factura vencida / gig-hecho-sin-factura), filas-chip de eventos `date`, quick-capture arriba, buzón de consentimiento IA (accept/dismiss). Prompts: `build/desk-prompt.md` + `build/desk-design-prompt.md`. El modelo de tareas YA ESTÁ VIVO (ADR-071). Decisión abierta anotada en el propio código (`h/desk/+page.svelte` L14-17): si la sección v1 de tasks se disuelve en el feed mixto — la decide Marco
- [ ] **Calendar v2 — despachado, 0 código** (ADR-072 + ADR-076). Sin migración: no existe `availability_block`, `conflictsFor()`, date en cascada ni la "Migration C" del gap #8 (user↔person). Orden: `build/calendar-model-prompt.md` (migración + APIs, PRIMERO) → `build/calendar-prompt.md` (UI: toggle month|list, gramática de holds outline/solid, menú "+" Performance|Date) → `build/calendar-design-prompt.md`. La UI viva sigue siendo la de Phase 0.2 (month grid + pins + diálogo ICS)

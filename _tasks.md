...
- [ ] **Calendar v2 — GRILLADO 2026-07-18 (ADR-078), 0 código** (ADR-072 + ADR-076 + ADR-078). Cerrado en el grill: diálogo unificado (el pill de tipo manda; forma de performance REUTILIZADA de `PerformanceCreateDialog`, no forkeada), blackout sin `kind` (persona? + rango + certeza + nota; UN espacio; select = equipo, no el libro), `day_off` nuevo en `date_kind` ("Dia off"), etiqueta custom `custom_fields.label` + autocomplete, pill Opció → tentative, inferencia fora en 2 niveles (v1 = determinista entre viajes, `awayBands()`; IA rellena datos que faltan, nunca calcula por encima), **paridad estricta IA=UI** (amendment ADR-069), `?view=agenda` SIN ruta alias, hora de entrada = local del venue (cierra el bug vivo de timezones vía forma compartida), ICS a overflow "⋯", stats de masthead se quedan. Orden: `build/calendar-model-prompt.md` (migración + APIs — **DESPACHABLE YA**, actualizado post-grill) → `build/calendar-prompt.md` (UI — **BLOQUEADO** hasta: Desk v2 aterrizado + diseño final de Marco) → diseño en refinamiento por Marco (contrato vinculante en `calendar-design-prompt.md § Binding contract`). La UI viva sigue siendo la de Phase 0.2 (month grid + pins + diálogo ICS)

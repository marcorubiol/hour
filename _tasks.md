...
## Queue
- [ ] **Aplicar la migración ADR-067** (`build/migrations/2026-07-16_workspace_shortid_alias.sql`) — short-id + alias flow. Vía Supabase MCP `apply_migration` (OAuth pendiente) o pegada en el SQL editor. Post-apply, en orden: (1) `UPDATE user_profile SET is_platform_admin = true WHERE user_id = (SELECT id FROM auth.users WHERE email = '<email de Marco en Hour>');` (2) quitar el TODO de `api/workspaces/+server.ts` añadiendo `alias` al select del GET; (3) regenerar db-types y borrar el tipo manual `alias?` de WorkspaceItem/NavWorkspace.
- [ ] **Verificar ADR-067 en navegador** (código listo, checks verdes — svelte-check 0/0, unit 110/110; el redirect de rutas viejas y el ?scope= solo se ven con sesión): logo→`/h` digest, `/h/desk|calendar|contacts|money`, `/h/muk-cia/desk`→redirect, pins→`?scope=` en la barra, Copy link, alias request en Edit space.

...
- **Reset v2 + roadsheet + section + account layer** (ADR-001..007 + ADR-023 + ADR-031 + ADR-032, 2026-04-19 → 2026-05-18): **24 tablas**. Polymorphic core: `account + workspace + project + engagement + show + section + date` — no `project.type` column (ADR-007). Engagement (conversation) distinct from show (atomic gig, ADR-001). **Section** es la agrupación operativa dentro del project, con `kind` enum de 10 valores: `tour | season | phase | circuit | residency | other | creation | campaign | comms | misc` (ADR-031 — renombrada desde `line` para abarcar casos no-touring). Money stack: `invoice + invoice_line + payment + expense` (ADR-003). Editable RBAC: `workspace_role` catalog + `project_membership.roles/grants/revokes` con 10-permission closed vocabulary (ADR-006). `venue` es su propia entidad.
...
### DB en producción
- `hour-phase0` · Postgres 17 · **24 tablas** + `show_redacted` view · 24 functions · 74 RLS policies (ENABLE + FORCE)

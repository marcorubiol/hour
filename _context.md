...
- **Scope por pins** (sustituye el sidebar-filtro de ADR-038): componente `ScopeStrip` en TODAS las lenses (pastilla "All spaces" sin pins). `resolveScope`/`inScope` en `$lib/nav.ts`.
- **Home** = próximos 7 días capado a 10 (`AgendaBoard.svelte`) + "+ N more → Agenda"; **nueva vista `/h/[ws]/agenda`** sin capar.

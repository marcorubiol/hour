# Hour — Build prompt: Contacts lens v1.5 (last contact + grouping)
...
> Origin: S1 2026-07-17, ADR-073. Spec: `build/screen-data-spec.md § Contacts`.

## Context

Read first: `apps/web/src/routes/h/contacts/+page.svelte` + `EngagementTable.svelte`
(filters, pagination, optimistic status PATCH, next-action dialog). Keep their semantics.
`engagement.first_contacted_at` / `last_contacted_at` exist in the schema since v2 —
nothing reads or writes them.

## Task

1. **"Last contact" column** in EngagementTable: `last_contacted_at` as a quiet relative
...
2. **Write path** (server, in the existing `/api/engagements` handlers):
...
3. **Grouping toggle** — "By conversation | By contact" (client-side, over the fetched
   ...
   their engagements) with their conversations as project chips (status-toned, click →

-- ADR-084 §1 — create a whole multi-day block in ONE call.
--
-- A week of rehearsals is N `date` rows sharing a series_id. Looping
-- `create_date` from the client would be N round-trips and, worse, NOT
-- atomic: a failure at day 3 leaves half a week on the calendar, and a
-- half-created block is exactly the kind of dirty state you cannot later
-- tell apart from a deliberate one. One statement, all or nothing.
--
-- The caller sends the EXACT timestamps, one per day (`p_starts`, and
-- optionally `p_ends` of the same length). Deliberately no server-side
-- "apply this time to those days" arithmetic: venue-local hour entry is
-- already solved client-side (ADR-078 §11) and duplicating that timezone
-- reasoning here would give the two paths two chances to disagree.
--
-- Same gate, same guards and the same claim-independence as create_date
-- (ADR-078 §7 — no write path bypasses the contract).
...
BEGIN
  ...
  IF (SELECT count(DISTINCT s::date) FROM unnest(p_starts) s) <> v_n THEN

...
Two confirmed synthetic Auth users with deliberately different access:
...
baseline is **114/114** with no skipped fixture cases.
...
- Emails: `playwright@hour.test` and `limited@hour.test`
...
### 2. Attach both identities (SQL Editor)
...
| Limited-role tests skip | `PW_LIMITED_EMAIL` / `PW_LIMITED_PASSWORD` missing from `.env.test`. |
...
- Two access envelopes test both successful operations and real authorization denials.
